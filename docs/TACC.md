...
## 3b. One-shot pipeline incl. encoder (single dev-queue job)

Everything through the DESK `validate` experiment fits in one 2-hour `development`
job. `scripts/tacc/pipeline.sh` defines the chain as selectable stages and
`00_pipeline.slurm` runs it:
...
squeue -u $USER ; tail -f houfin_pipeline.o<jobid>
```

Stage order (each `run_encoder.py` stage is its own process):
`preprocess → climate → climate_grid → states → ebird_cache → bbs → amplitude →
esk → desk → cube → validate`. The last four are the encoder: eBird ESK Z →
N-stream DESK (env→Z) → per-year Z cube → the validate report
(`$HOUFIN_PROCESSED/encoder/desk/validate_report.json`: CKA/Mantel/Pearson per
period — how far back eBird-only extrapolation reproduces the BBS structure).

Select a subset with `STAGES` (skip stages already done). If preprocess + climate
already ran, run just the new + encoder half:

```bash
STAGES="climate_grid states ebird_cache bbs amplitude esk desk cube validate" \
  bash scripts/tacc/submit_pipeline.sh
```

For `bbs_mode=off` (original eBird-only, no BBS), drop `bbs amplitude validate`.
`climr` from cold is the only stage that can blow the 2h window; if its cache
isn't warm, run `climate` once on `normal` (`QUEUE=normal TIME=06:00:00 …`) or a
login node first, then the rest on dev.
...
clear any stale encoder artifacts (states, ESK/DESK/cube outputs, old
`env_model_semisup.pth`) so nothing mixes formats:

```bash
rm -rf $HOUFIN_PROCESSED/encoder            # regenerated by states/ebird_cache/esk/desk/cube
# raw downloads + 25 km products under $HOUFIN_DATA are kept (preprocess reuses them)
```

Also confirm the BBS release has `bbs_2026_release/{SpeciesList.csv,Weather.csv,
