...
- `src/data/combine/streams.py` is the ETL step feeding DESK: a config-driven registry of covariate streamers — the five in `states.streams` are a monthly bio-year **climate** stream, a **land-use** stream (LUH-3), a **HYDE** stream, a CONUS-only **BUI** stream (HISDAC-US built-up intensity, carrying its own availability channel), and static **soil** and **elevation** streams. The per-year streams linearly interpolate between snapshots for the sparse products (BUI is 5-yearly, HYDE decadal before 1951), holding constant rather than extrapolating outside the snapshot range. They iterate in lockstep, apply a 2-year exponential moving average (`ema_tau`, reduced from 10 so `z_raw` fluxes and the two lags stay identifiable), and write one `state_{year}.npz` (one named array per stream) plus a training-vector bag. The registry is stream-agnostic, which is how a covariate is added or removed: `states.streams` is the authoritative list and has no code-side default.
...
`scripts/viz/validation_report.py` is the figure suite for the *validation* JSONs, which had never been plotted. It reads `validate_report.json`, `bbs_route_validation.json` and `bbs_epoch_neighborhood.json` out of a run directory and writes numbered PNGs plus a self-contained `index.html`, ordered as an argument: the experiment's design, the space/time/both scoreboard, then **the room chart** — floor to independent ceiling with DESK placed inside it, which is what turns a bare skill of 0.011 into a share of the signal that was resolvable at all — then the noise ceiling, the baseline ladder by era, decay against extrapolation distance, the direction/magnitude geometry against its MSE-optimal locus, co-movement by separation, and the 64-component spectrum. Two rules are enforced in code rather than left to the reader: `esk_truncation` is never drawn as a ceiling (it shares the target's noise draw, so `resolving_room`'s fallback to it overstates the room by roughly half — the honest split-half ceiling lives in the sibling `ceiling` block and is read from there), and a structurally unavailable rung is hatched with its reason rather than rendered as a zero. `--arms <run dirs>` instead draws the sweep's designed measurement: skill against extrapolation distance on `common_holdout_years`, the only window every overlay run withheld. It is run by hand, like the rest of `scripts/viz/`, and needs no GPU, no config and no data tree — only the JSONs. `scripts/viz/_validation_style.py` pins one colour per predictor across every figure; `scripts/viz/_validation_load.py` imports `resolving_room`, `NULL_IS_A_FLOOR_FOR` and `PREDICTOR_DENOISING` from the validators rather than restating what counts as a floor.

`scripts/viz/validation_maps.py` is the same results as **maps of North America**, because the questions this project is stuck on are spatial: is the error at the expansion front, is it inside the Great Plains barrier where `Z_disp` feeds `Q` and nothing downstream is fitted to correct it, is the coastal pattern ecology or sampling. Eight maps — the split on the ground, **where the experiment has any power at all** (the per-cell split-half noise floor, which no scalar reports), which ladder rung wins where, DESK against the honest bar at held-out *block* resolution, direction and magnitude against each cell's own ceiling, error by Great Plains zone, extrapolation depth in km, and the error through time. The design constraint is that a naive map discards every device the metric suite uses to stay honest, so each one carries at least one over: a tie fades to the background at the measured 6.6% seed spread rather than claiming a winner; a cell with too few surveys to split in half is refused with its reason rather than coloured; the era panels share one scale while panels of different quantities never do; and held-out results aggregate to the 6×6 block, because 217 held-out cells sit in ~87 contiguous blocks and within-block texture is not evidence. `scripts/viz/_geo.py` is the shared geo context that made this possible — projected ESRI:102003 extent, Natural Earth coastline, Great Plains outline from the HPC-safe zone *raster*, and a scale bar; it deliberately reads the committed 27 km grid rather than the 25 km local override. `scripts/build_colonization_front.py` derives the observed first-detection year that the ecological overlay needs, masking the native western hull because pseudo-zeros are only asserted outside a 700 km halo and a western cell's "first detection" is BBS's launch year rather than a colonization date.

The validators now emit a **map layer** to make all of this possible: the per-cell noise floor and the ceiling table's own per-cell arrays (both were computed and reduced to a median), the ladder's six per-row error arrays, `compare_positions`' per-row error and magnitude/angular split for every predictor, and the direction panel's per-cell dir-cos, magnitude ratio, window depth and split-half ceiling — `median_dir_cos` and `_ceiling_row` both destroyed those vectors internally and now have return-the-vector siblings. `validate_spacetime.extract_map_layer` splits the scalars from the arrays in one traversal on the way to disk, keyed on "an ndarray is not a scalar" rather than a list of key names, since an allow-list is what let the interpolation bar be computed and never reported.
