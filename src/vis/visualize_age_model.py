...
def reconstruct_simulation(data, params):
    ...
    for k, v in params.items():
        if k.endswith("_auto_loc"):
            base_key = k.replace("_auto_loc", "")
            cleaned_params[base_key] = v
        else:
            cleaned_params[k] = v

    # Add a leading batch/sample dimension [None, ...] so Predictive executes a clean vector pass
    posterior_samples = {
        k: v[None, ...] if (v.ndim == 0 or v.shape[0] != 1) else v 
    ...
    # Strip the dummy sample/batch axis off the returned dictionary arrays
...
def rebuild_age_pools(sim, data, params):
    ...
    time = sim['simulated_density'].shape[0]
    
    # Pre-allocate zero grid for scattering parameters to 2D
    zero_grid = jnp.zeros((Ny, Nx))
    
    # Extract the flat parameters generated by the predictive trace
    Sa_flat = sim['Sa_flat']
    Sj_flat = sim['Sj_flat']
    Fmax_flat = sim['Fmax_flat']
    K_flat = sim['K_flat']
    Q_flat = sim['Q_flat']
    
    # Pre-calculate the flat density-dependence brake field
    denominator_c = 1.0 - Sa_flat + 1e-6
    c_flat = jnp.maximum((Fmax_flat * Sj_flat) / denominator_c - 1.0, 0.0)
    
    # Extract the learned global Allee coefficient
    allee_gamma = sim['allee_gamma']
    
    # Pull the baseline initialization vectors directly from data
    init_N_a = data['initpop_latent'] * 0.5 
    init_N_j = data['initpop_latent'] * 0.5

    # Extract global variables needed for the dispersal step function signature
    dispersal_logit_intercept = sim['dispersal_logit_intercept']
    dispersal_logit_slope = sim['dispersal_logit_slope']
    ...
    def step(pools, t):
        ...
        # 1. Handle invasion introduction mechanics using the resolved vector
        row, col = data['inv_location']
        k = t - data['inv_timestep']
        is_invading = (k >= 0) & (k < inv_pop_vector.shape[0])
        val = jnp.where(is_invading, inv_pop_vector[jnp.minimum(jnp.maximum(0, k), inv_pop_vector.shape[0]-1)], 0.0)
        N_a = N_a.at[row, col].add(val * 0.5)
        N_j = N_j.at[row, col].add(val * 0.5)
        
        # 2. Scatter the 1D timeline slices out into full 2D grids
        Sa_g = zero_grid.at[land_rows, land_cols].set(Sa_flat[t])
        Sj_g = zero_grid.at[land_rows, land_cols].set(Sj_flat[t])
        Fmax_g = zero_grid.at[land_rows, land_cols].set(Fmax_flat[t])
        K_g = zero_grid.at[land_rows, land_cols].set(K_flat[t])
        c_g = zero_grid.at[land_rows, land_cols].set(c_flat[t])
        Q_g = zero_Q_grid.at[:, land_rows, land_cols].set(Q_flat[t].T)
        ...
        # 4. Apply vital rates and pre-calculated density brakes
        ...
        # 5. Mask boundaries and enforce positivity constraints
...
def plot_age_structure_dynamics_map(Na_grid, Nj_grid, years, map_params, output_dir, land_mask):
    """
    MAP Version: Visualizes shifting realized age structure snapshots and compares
    the continental trajectory against the fixed theoretical potential at K derived from point estimates.
    """
    ...
    Sa_base = sigmoid(map_params['alpha_a'])
    Sj_base = sigmoid(map_params['alpha_j'])
    Fmax_base = softplus(map_params['alpha_f'])

    # Reconstruct dynamic 'c' calculation at carrying capacity equilibrium
    c_dynamic = (Fmax_base * Sj_base) / (1.0 - Sa_base + 1e-6) - 1.0
    c_dynamic = max(c_dynamic, 0.0)
    
    # Realized fecundity at K
    F_at_K = Fmax_base / (1.0 + c_dynamic)
    
    # Compute baseline demographic target line
    ...
    plt.plot(years, global_rho_realized, color='forestgreen', linewidth=2, label="Realized (Actual Population)")
    plt.axhline(global_rho_theoretical, color='navy', linestyle=':', linewidth=2, 
                label=f"Theoretical Potential at K ({global_rho_theoretical:.2f})")
    plt.axhline(0.5, color='black', linestyle='--', alpha=0.3, label="Prior Target Anchor (0.50)")
    
    plt.title(r"Continental Juvenile Fraction ($\rho = N_j / N_{total}$)")
    plt.xlabel("Year")
    ...
    for ax, year in zip(axes.flatten(), snapshots):
        ...
        # Swapped to 'RdYlBu' to visually align with the SVI spatial maps, 
        # setting the midpoint tightly around the model's theoretical baseline
        im = ax.imshow(rho_masked, cmap='RdYlBu', origin='upper', vmin=0.3, vmax=0.7)
        ax.set_title(f"Realized Juvenile Fraction - {year}", fontsize=14)
        ax.axis('off')

    cbar_ax = fig.add_axes([0.15, 0.05, 0.7, 0.02])
    cbar = fig.colorbar(im, cax=cbar_ax, orientation='horizontal')
    cbar.set_label(r"Realized Juvenile Fraction ($\rho$)", labelpad=10, fontsize=12)
    
    # Explicit line on colorbar mapping where the point-estimate anchor settled
    cbar.ax.axvline(global_rho_theoretical, color='navy', linestyle=':', linewidth=2)
    
    plt.suptitle("Spatiotemporal Realized Age Structure Dynamics (MAP)", fontsize=18, y=0.95)
...
def analyze_source_sink_mortality(data, Sa_grid, Sj_grid, Fmax_grid, Q_grid, output_dir, allee_gamma, pop_scalar):
    print("Analyzing Theoretical Source-Sink Dynamics with Temporal Panels...")
    land_mask = data['land_mask']
    T_steps = Sa_grid.shape[0]
    
    if T_steps < 20:
        raise ValueError(f"Simulation time series length ({T_steps}) must be at least 20 years to compute 10-year decade windows.")

    # 1. Scale the Allee parameter and calculate the low-density founding ceiling
    N_founding_relative = 1.0 / pop_scalar
    allee_factor_founding = 1.0 - np.exp(-allee_gamma * N_founding_relative)
    
    # 2. Compute Allee-Corrected Local Theoretical R0 across all individual years
    # Grid shapes: (T, Ny, Nx)
    F_unconstrained_grid = Fmax_grid * allee_factor_founding
    R0_grid = (F_unconstrained_grid * Sj_grid) / (1.0 - Sa_grid + 1e-6)
    
    # 3. Calculate temporal windows
    R0_full_avg = np.mean(R0_grid, axis=0)
    R0_first_10 = np.mean(R0_grid[:10], axis=0)
    R0_last_10  = np.mean(R0_grid[-10:], axis=0)
    
    # Mask all fields to valid land boundaries
    R0_full_masked  = np.ma.masked_where(land_mask == 0, R0_full_avg)
    ...
    im_r0 = plt.imshow(R0_full_masked, cmap='RdYlBu', origin='upper', norm=mcolors.LogNorm(vmin=vmin_val, vmax=vmax_val))
    cbar = plt.colorbar(im_r0, label="Fundamental Reproductive Rate ($R_0$)", extend='both')
    cbar.ax.axhline(1.0, color='black', linewidth=2, linestyle='--')
    plt.title("Continuous Intrinsic $R_0$ Landscape\n(Full Timeline Average)")
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, "analysis_source_sink_continuous.png"), dpi=200, bbox_inches='tight')
    plt.close()

    # Decade Comparison Panels + Linear Absolute Delta
    fig, axes = plt.subplots(1, 3, figsize=(24, 7))
    
    # First 10 Years (Log Scale)
    im0 = axes[0].imshow(R0_first_masked, cmap='RdYlBu', origin='upper', norm=mcolors.LogNorm(vmin=vmin_val, vmax=vmax_val))
    fig.colorbar(im0, ax=axes[0], fraction=0.046, pad=0.04).ax.axhline(1.0, color='black', linestyle='--')
    axes[0].set_title("First 10 Years Mean ($R_0$)")
    axes[0].axis('off')
    
    # Last 10 Years (Log Scale)
    im1 = axes[1].imshow(R0_last_masked, cmap='RdYlBu', origin='upper', norm=mcolors.LogNorm(vmin=vmin_val, vmax=vmax_val))
    fig.colorbar(im1, ax=axes[1], fraction=0.046, pad=0.04).ax.axhline(1.0, color='black', linestyle='--')
    axes[1].set_title("Last 10 Years Mean ($R_0$)")
    axes[1].axis('off')
    
    # Absolute Linear Delta Grid
    R0_delta = R0_last_10 - R0_first_10
    R0_delta_masked = np.ma.masked_where(land_mask == 0, R0_delta)
    max_delta = max(float(np.ma.max(np.abs(R0_delta_masked))), 0.1)
    
    # 'bwr' scales negative shifts to red (degradation) and positive to blue (improvement)
    im2 = axes[2].imshow(R0_delta_masked, cmap='bwr', origin='upper', vmin=-max_delta, vmax=max_delta)
    fig.colorbar(im2, ax=axes[2], fraction=0.046, pad=0.04, label="Absolute Linear Shift ($\Delta R_0$)")
    axes[2].set_title("Absolute Delta (Last 10 - First 10)")
    axes[2].axis('off')
    
    plt.suptitle("Decadal Dynamics of Continuous Habitat Viability", fontsize=16, y=0.98)
    ...
    # Full Timeline Baseline
    plt.figure(figsize=(10, 8))
    plt.imshow(binary_full, cmap=cmap_binary, origin='upper')
    legend_elements = [
        mpatches.Patch(color='#4575b4', label='Theoretical Source ($R_0 > 1$)'),
        mpatches.Patch(color='#d73027', label='Theoretical Sink ($R_0 \leq 1$)')
    ]
    plt.legend(handles=legend_elements, loc='lower right', frameon=True)
    plt.title("Binary Theoretical Source-Sink Landscape\n(Full Timeline Average)")
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, "analysis_source_sink_binary.png"), dpi=200)
    plt.close()

    # Decade Comparison Panels + Categorical Flip Transitions
    fig, axes = plt.subplots(1, 3, figsize=(24, 7))
    
    axes[0].imshow(binary_first, cmap=cmap_binary, origin='upper')
    axes[0].set_title("First 10 Years Status")
    axes[0].axis('off')
    
    axes[1].imshow(binary_last, cmap=cmap_binary, origin='upper')
    axes[1].set_title("Last 10 Years Status")
    axes[1].axis('off')
    
    # Compute regime flips
    #  +1 = Opened up (Sink -> Source), 0 = Static, -1 = Collapsed (Source -> Sink)
    binary_delta = binary_last - binary_first
    binary_delta_masked = np.ma.masked_where(land_mask == 0, binary_delta)
    cmap_flips = mcolors.ListedColormap(['#d73027', '#e0e0e0', '#4575b4']) # Red=Loss, Grey=No Change, Blue=Gain
    
    axes[2].imshow(binary_delta_masked, cmap=cmap_flips, origin='upper', vmin=-1, vmax=1)
    legend_flips = [
        mpatches.Patch(color='#4575b4', label='Gained Source Status (Sink $\\rightarrow$ Source)'),
        mpatches.Patch(color='#e0e0e0', label='No Regime Change'),
        mpatches.Patch(color='#d73027', label='Lost Source Status (Source $\\rightarrow$ Sink)')
    ]
    axes[2].legend(handles=legend_flips, loc='lower right', frameon=True)
    axes[2].set_title("Regime Flips over Time")
    axes[2].axis('off')
    
    plt.suptitle("Decadal Regime Transitions of Binary Source-Sink Boundaries", fontsize=16, y=0.98)
    ...
    def build_ternary_layer(r0_layer):
        t_map = np.zeros_like(r0_layer)
        t_map = np.where(r0_layer > upper_thresh, 2, t_map)
        t_map = np.where((r0_layer >= lower_thresh) & (r0_layer <= upper_thresh), 1, t_map)
        t_map = np.where(r0_layer < lower_thresh, 0, t_map)
        return np.ma.masked_where(land_mask == 0, t_map)

    ternary_full  = build_ternary_layer(R0_full_masked)
    ternary_first = build_ternary_layer(R0_first_masked)
    ternary_last  = build_ternary_layer(R0_last_masked)

    # Full Timeline Baseline
    plt.figure(figsize=(10, 8))
    plt.imshow(ternary_full, cmap=cmap_ternary, origin='upper')
    plt.legend(handles=legend_elements_ternary, loc='lower right', frameon=True)
    plt.title(f"Core Theoretical Source-Sink Landscape ($\pm 10\%$ Threshold)")
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, "analysis_source_sink_core.png"), dpi=200)
    plt.close()

    # Decade Comparison Panels + Arithmetic Change
    fig, axes = plt.subplots(1, 3, figsize=(24, 7))
    
    axes[0].imshow(ternary_first, cmap=cmap_ternary, origin='upper')
    axes[0].legend(handles=legend_elements_ternary, loc='lower right', frameon=True)
    axes[0].set_title("First 10 Years Quality Class")
    axes[0].axis('off')
    
    axes[1].imshow(ternary_last, cmap=cmap_ternary, origin='upper')
    axes[1].legend(handles=legend_elements_ternary, loc='lower right', frameon=True)
    axes[1].set_title("Last 10 Years Quality Class")
    axes[1].axis('off')
    
    # Ternary Categorical Shift Map
    ternary_delta = ternary_last.astype(float) - ternary_first.astype(float)
    axes[2].imshow(ternary_delta, cmap='bwr', origin='upper', vmin=-2, vmax=2)
    fig.colorbar(axes[2].images[0], ax=axes[2], fraction=0.046, pad=0.04, label="Class Rank Vector Shift")
    axes[2].set_title("Categorical Class Shifts")
    axes[2].axis('off')
    
    plt.suptitle("Decadal Quality Class Shifting of Core and Marginal Habitats", fontsize=16, y=0.98)
    ...
    k_stack_fft = data['juvenile_fft_kernel_stack']
    edge_corr = data['juvenile_edge_correction_stack']
...
def plot_results():
    ...
    map_demographics = {
            'alpha_a': params.get('alpha_a', params.get('alpha_a_auto_loc', sim.get('alpha_a'))),
            'alpha_j': params.get('alpha_j', params.get('alpha_j_auto_loc', sim.get('alpha_j'))),
            'alpha_f': params.get('alpha_f', params.get('alpha_f_auto_loc', sim.get('alpha_f'))),
        }
    ...
    plot_age_structure_dynamics_map(
        Na_grid, Nj_grid, years, map_demographics, OUTPUT_PLOT_DIR, data['land_mask']
    )
    analyze_source_sink_mortality(
        data, Sa_grid[start_idx:], Sj_grid[start_idx:], Fmax_grid[start_idx:], Q_grid[start_idx:], 
        OUTPUT_PLOT_DIR, allee_gamma_learned, data['pop_scalar']
    )
